...
| 机会 | Tier1 分组信号：**稳健股** RSI(14)<20 强买入★ / <25 买入、PE分位>95且RSI>70 卖出；**动量股** 破布林下轨且价在MA200上 买入。悬停看依据 | ✅ 实时 |
...
### 推送的信号
**Tier1 分组**：稳健股(非HI_VOL) 🟢 强买入 `RSI(14)<20` / 买入 `RSI(14)<25`、🟠 卖出 `PE分位>95且RSI>70`；动量股(HI_VOL) 🟢 买入 `破布林下轨且价在MA200上`。至少配一个通知渠道，未配则只在日志打印。
