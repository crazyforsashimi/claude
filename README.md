...
| ETF | QQQ 纳指100 · SPY 标普500 · SOXX 半导体 · XLU 公用事业 |
...
- **ETF（QQQ/SPY/SOXX/XLU）** 不申报财报，无 PE/分位，仅显示价格类指标；卖出信号（需 PE 分位）与财报事件闸门对它们天然不触发。
...
`python3 build_dataset.py` 为 32 支自选标的各生成一份 `historical_data/{TICKER}.csv`，每行一个交易日，55列，分四类：
...
`python3 build_labels.py` 在 `historical_data/` 的干净日线上，产出 **`model_dataset.csv`**（32 支标的 pooled 堆叠，39129 行 × 81 列，含 `ticker` 列），供 LightGBM/XGBoost 训练**买入/卖出高精度信号**。定位：`build_dataset.py` 提供原始特征与前瞻收益，本脚本在其上加**可交易的标签**和**建模就绪的相对特征**。
...
- ETF(QQQ/SPY/SOXX/XLU) 不申报财报 → 估值列全部留空，不冒充。
- BRK.B 无摊薄EPS → 估值列留空。
...
云端定时、免费、不用电脑开着：每个美股交易日收盘后自动拉 32 标的数据、检测大机会信号，**有触发就推微信/邮件，无触发不打扰**。信号口径与工具、回测完全一致（复用 `build_dataset`）。
...
## 变更记录
- **v33（2026-08-06）**：**新增 XLU（公用事业 ETF）入池，标的数 31 → 32**，全管线重跑（数据刷新至 2026-08-06）。XLU 是低波动均值回归型标的（年化波动 ~14%），与池内科技/半导体高度负相关（近20日 XLU vs XLK 相关 −0.10），补上防守端观察位。校准结果：**5年 RSI&lt;20 强买入★ / RSI&lt;22 买入**（5年仅触发 1/2 次，极端罕见）；**2年 RSI&lt;28 强买入★ / RSI&lt;30 买入 · 破150日布林 强买入★ / 破20日布林 买入**（后者近两年 21 次 8 个独立事件，5/10/20 日涨率 76/81/81%）。作为 ETF 无财报 → PE 分位、卖出信号、财报事件闸门天然不触发，只走 RSI/布林买入档。**⚠️ 2年窗口的「破20日布林」档要打折看**：同一条件放到 5 年窗口只有 60% 胜率（N=63），近两年 81% 主要来自 XLU 这两年 +20% 的上涨市 beta；5年样本里 2022-09、2023-09 两次失败都出现在长端收益率单边上行阶段——利率环境是这个标的超卖信号的主要失效场景。同批重跑另有 **BABA 由「无信号」获得 2年档**、10 只标的阈值随数据窗口滚动微调（AMZN/CAT/CEG/GOOGL/GS/LLY/MSFT/QCOM/QQQ/SPY），现 **31 只有档、1 只无信号（COIN）**。
...
- **v18（2026-07-21）**：新增每日大机会告警 `daily_alert.py` + GitHub Actions 定时(`.github/workflows/daily-alert.yml`)。每交易日收盘后云端自动检测 32 标的信号(复用 `build_dataset`、口径一致)，有触发推微信 Server酱/邮件、无触发不打扰。配置见上节。
