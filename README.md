...
## 变更记录
- **v27（2026-07-22）**：①邮件标题改为**「自选股机会信号提示：买入N·卖出M」**(强买入并入买入计数、正文仍分组)。②**事件闸门补财报当天盲区**：财报当天新季 `filing_date` 还没进数据源(T+1~数天入库)、Starter 又拿不到前瞻财报日历(Benzinga 403)，故"财报当天暴雷"实时认不出(GEV 今日 −7.7% 财报后破下轨触发买入即漏标)。**不用"当天大阴线"兜底**——回测证纯大阴线抄底反更准(单日−8%达80%)、只有财报造成的才有毒，兜底会误伤好机会。折中：距最近已知财报 >85 天(下季可能已发未入库)且触发买入时，附**中性提示 ℹ️「财报数据仅到X、请核对是否刚发财报」**(不标坑、不改判断、不误伤)。③代码瘦身：删 `index.html` 死代码(`hi52`/`distHi`，旧减仓规则遗留)、README 大机会段去重(过时 v12 数字表 → 指向 logic.html)。**说明**：两次 run 结果不同(GEV 一次触发一次否)是**盘中扫描固有特性**——cron 有延迟、当天日线 bar 收盘前不定盘，信号随盘中价变动，非 bug。
