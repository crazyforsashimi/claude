...
def group_label(tk):
    """按信号来源标注(两套合并看有无 RSI/布林) → (标签文字, 背景色, 文字色)。"""
    cfg = SIGNAL_CFG.get(tk, {})
    has_rsi = has_boll = False
    for w in ("5y", "2y"):
        c = cfg.get(w, {})
        has_rsi = has_rsi or bool(c.get("rsi", {}).get("s") or c.get("rsi", {}).get("b"))
        has_boll = has_boll or bool(c.get("boll", {}).get("s") or c.get("boll", {}).get("b"))
